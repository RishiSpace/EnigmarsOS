...
if is_fat "${BOOT_FSTYPE}" && [[ "$(findmnt -no TARGET /boot 2>/dev/null)" == "/boot" ]] \
  ...
  KPATH="boot():/EFI/EnigmaOS/vmlinuz-linux"
  IPATH="boot():/EFI/EnigmaOS/initramfs-linux.img"
  FPATH="boot():/EFI/EnigmaOS/initramfs-linux-fallback.img"
  UCODE_AMD="boot():/EFI/EnigmaOS/amd-ucode.img"
  UCODE_INTEL="boot():/EFI/EnigmaOS/intel-ucode.img"
fi
...
if [[ -f "${LIMINE_DATA}/limine-bios.sys" ]]; then
  ...
  install -Dm644 "${LIMINE_DATA}/limine-bios.sys" "${ESP}/EFI/EnigmaOS/limine-bios.sys" || true
...
write_conf "${ESP}/EFI/EnigmaOS/limine.conf"
